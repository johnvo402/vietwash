...
namespace Presentation.Endpoints.Customer
{
    ...
    : EndpointBaseAsync.WithRequest<ListCustomerQuery>.WithActionResult<ApiResponse<PaginationResponse<ListCustomerResponse>>>
    {
        [HttpGet(Router.Customer.GetList)]
        [SwaggerOperation(Tags = [Router.Customer.Tags], Summary = "list Customer")]
        //[AuthorizeBy]
        public override async Task<ActionResult<ApiResponse<PaginationResponse<ListCustomerResponse>>>> HandleAsync(
