...
public class ProjectionSpecificationEvaluator
{
    public static IQueryable<TResponse> GetQuery<T, TResponse>(
        ...
        where T : class
        where TResponse : class => Evaluate(inputQuery, specification);

    public static string SpecStringQuery<T, TResponse>(ISpecification<T, TResponse> specification)
        where T : class
        where TResponse : class =>
        Evaluate(Enumerable.Empty<T>().AsQueryable(), specification).Expression.ToString();
        ...
        where TResponse : class
    {
        ...
        if (specification.Sorts.Count > 0)
        {
            ...
            {
                OrderByInfo<T> orderbyInfo = orderByInfos[i];
                if (order == null)
                    order = orderbyInfo.OrderType == OrderType.Asc
