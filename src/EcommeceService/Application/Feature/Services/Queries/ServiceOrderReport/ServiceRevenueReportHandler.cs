...
namespace Application.Feature.Services.Queries.ServiceOrderReport
{
    public class ServiceRevenueReportHandler(IUnitOfWork unitOfWork)
        : IRequestHandler<ServiceRevenueReportQuery, Result<List<ServiceRevenueReportResponse>>>
    {
        public async ValueTask<Result<List<ServiceRevenueReportResponse>>> Handle(
        ...
        )
        {
            try
            {
                ...
                var orders = await unitOfWork
                    ...
                    .ListAsync(
                        new GetOrdersForServiceRevenueReportSpecification(
                            request.StartDate,
                            request.EndDate
                        ),
                        queryParamRequest,
                ...
                foreach (var serviceOrderItem in serviceOrderItems)
                {
                    ...
                    var orderitemGrossRevenue = serviceOrderItem.OrderItems.Sum(oi =>
                    ...
                    foreach (var order in serviceOrderItem.Orders)
                    {
                        ...
                        var groupGrossRevenue = serviceOrderItem
                    ...
                    reports.Add(
                        new ServiceRevenueReportResponse
                        {
                ...
                return Result<List<ServiceRevenueReportResponse>>.Success(reports);
            }
            catch (Exception ex)
            {
                throw;
