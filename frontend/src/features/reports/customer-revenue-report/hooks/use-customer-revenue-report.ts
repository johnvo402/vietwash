...
import { CustomerRevenueResult } from "@/api/generated";
import { useAuth } from "@/hooks/use-auth";
import { useQueryFilter } from "@/lib/filter";
import { useQuery } from "@tanstack/react-query";
import { useQueryState, useQueryStates } from "nuqs";

export const useCustomerRevenueReport = () => {
  ...
  const params = {
    ...
    // searchTargets: ["phoneNumber", "displayName"],
  };
  ...
  const {
    ...
    queryFn: async () => {
      const response =
        await apiClient.ecommerceApiReportRouteCustomerRevenueGet(...args);
      return {
        customerRevenueReport: response.data.results?.data || [],
  ...
  const fetchAllData = async () => {
    let allData: CustomerRevenueResult[] = [];
    let currentPage = 1;
    const pageSize = 5000;
    ...
    while (true) {
      ...
      const response =
        await apiClient.ecommerceApiReportRouteCustomerRevenueGet(...fetchArgs);
  ...
  return {
    customerRevenue: data?.customerRevenueReport || [],
    fetchAllData,
    paging: data?.paging || {},
