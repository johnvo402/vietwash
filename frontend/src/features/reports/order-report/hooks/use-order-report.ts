...
export const useOrderReport = () => {
  ...
  const {
    ...
    queryKey: ["orderReport", { page, pageSize, search, branchIds, from, to }],
    queryFn: async () => {
      const response = await apiClient.ecommerceApiReportRouteOrderGet(...args);
      return {
        orderReport: response.data.results?.data || [],
        paging: response.data.results?.paging || {},
      };
    },
  });
  const fetchAllData = async () => {
    let allData: OrderSummaryResult[] = [];
    let currentPage = 1;
    const pageSize = 5000;
    ...
    while (true) {
      ...
      const response = await apiClient.ecommerceApiReportRouteOrderGet(
        ...fetchArgs
      );
