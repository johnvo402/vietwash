...
export const useSupplierReport = () => {
  ...
  const {
    ...
    queryFn: async () => {
      const response =
        await apiClient.ecommerceApiReportRouteProductSupplierReportGet(
          ...args
        );
      return {
        supplierReport: response.data.results?.data || [],
        paging: response.data.results?.paging || {},
      };
    },
  });
  const fetchAllData = async () => {
    let allData: ProductSupplierReportResponse[] = [];
    let currentPage = 1;
    const pageSize = 5000;
