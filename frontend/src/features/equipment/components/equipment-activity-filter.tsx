...
import { TypeActivity } from "@/api/generated/api";
import { Filter } from "lucide-react";

interface EquipmentActivityFiltersProps {
...
export function EquipmentActivityFilters({
  time,
  typeFilter,
  refetch,
  onApply,
}: EquipmentActivityFiltersProps) {
    ...
    <div className="flex gap-4">
      <DropdownMenu open={open} onOpenChange={setOpen}>
        ...
        <DropdownMenuContent className="w-64">
          <DropdownMenuLabel>
            {t("equipment.activityType.title")} {/* Ví dụ: "Loại hoạt động" */}
          </DropdownMenuLabel>
          <Select value={type} onValueChange={setType}>
            <SelectTrigger className="mb-2">
              <SelectValue
                placeholder={t("common.filterBy", {
                  entity: t("equipment.activityType.title").toLowerCase(),
                })}
              />{" "}
              {/* Ví dụ: "Lọc theo loại hoạt động" */}
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="all">
                {t("equipment.activityType.allTypes")}{" "}
                {/* Ví dụ: "Tất cả loại hoạt động" */}
              </SelectItem>
              <SelectItem value={TypeActivity.Maintenance}>
                {t("equipment.activityType.maintenance")}{" "}
                {/* Ví dụ: "Bảo trì" */}
              </SelectItem>
              <SelectItem value={TypeActivity.Repair}>
                {t("equipment.activityType.repair")} {/* Ví dụ: "Sửa chữa" */}
              </SelectItem>
            </SelectContent>
          </Select>
          <DropdownMenuSeparator />
