...
public class UserSynchronizationTests
{
    ...
    public async Task SameCreateEventTwice_SucceedsAndPersistsOneUserWithAuthId()
    {
        ...
        CreateUserCommand command = CreateCommand(100);

        PubSubResponse<CreateUserCommand> first = await harness.Handle(command);
        PubSubResponse<CreateUserCommand> duplicate = await harness.Handle(command);
    ...
    public async Task UnrelatedDatabaseViolation_RemainsTransientFailure()
    {
        ...
        PubSubResponse<CreateUserCommand> result = await harness.Handle(CreateCommand(100));
    ...
    private static CreateUserCommand CreateCommand(long id) =>
        new()
        {
    ...
    private sealed class UserSyncHarness
    {
        ...
        public async Task<PubSubResponse<CreateUserCommand>> Handle(CreateUserCommand command)
        {
            var handler = new CreateUserHandler(unitOfWork.Object);
